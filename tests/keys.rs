...
use anyhow::Result;
use bitmask_core::{
    constants::switch_network, get_wallet_data, save_mnemonic, structs::SecretString,
    util::init_logging,
};
...
pub async fn taproot() -> Result<()> {
    ...
    let wallet_data = get_wallet_data(&decrypted_wallet.public.btc_descriptor_xpub, None).await?;

    // Compare to those generated by Sparrow Wallet
    assert_eq!(
        decrypted_wallet.public.btc_descriptor_xpub.to_string(),
