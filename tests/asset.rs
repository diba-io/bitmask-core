...
use bitmask_core::{
    ...
    get_network, get_wallet_data, import_asset, save_mnemonic_seed, send_assets, send_sats,
    transfer_many_assets, TransfersRequest,
};
use log::{debug, info};
...
async fn allow_transfer_one_asset_to_one_beneficiary() -> Result<()> {
    ...
    let resp = transfer_many_assets(TransfersRequest {
...
#[tokio::test]
async fn allow_transfer_many_assets_to_one_beneficiary() -> Result<()> {
    if env::var("RUST_LOG").is_err() {
        env::set_var(
            "RUST_LOG",
            "bitmask_core=debug,bitmask_core::operations::rgb=trace,asset=debug",
...
#[tokio::test]
async fn allow_transfer_many_assets_to_many_beneficiary() -> Result<()> {
    if env::var("RUST_LOG").is_err() {
        env::set_var(
            "RUST_LOG",
            "bitmask_core=debug,bitmask_core::operations::rgb=trace,asset=debug",
