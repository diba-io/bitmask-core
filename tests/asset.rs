...
use bitmask_core::{
    ...
    save_mnemonic_seed, send_assets, send_sats, rgb::shared::ContractType
};
use log::{debug, info};
...
/// Test asset import
#[tokio::test]
async fn fungible_transfer() -> Result<()> {
    ...
    let issued_asset = &create_asset(
        ...
        &ContractType::Fungible.to_string()
    )?;
...
async fn uda_transfer() -> Result<()> {
    ...
    let consignment_details = send_assets(
        &tmp_vault.btc_descriptor_xprv,
        &tmp_vault.btc_change_descriptor_xprv,
        &tmp_vault.rgb_udas_descriptor_xprv,
        &tmp_vault.rgb_udas_descriptor_xpub,
        &blinded_utxo.conceal,
        100,
        &issued_asset.genesis,
