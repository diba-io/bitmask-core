...
#[ignore = "Just running locally"]
#[tokio::test]
pub async fn measure_batch_operation() -> Result<()> {
    // 1. Initial Setup
...
pub async fn create_batch_transfer() -> Result<()> {
    ...
    let PublishedPsbtResponse { txid, sign, .. } = psbt_resp?;
    ...
    let (_txid, _transfer) = extract_transfer(consig.clone())?;
